...
import { calculateResults, QuizResult, ReadinessTier } from '@/lib/scoring';
...
function ResultsContent() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('axon-quiz-result');
      if (!stored) {
        setError('Assessment results not found. Please take the quiz again.');
        return;
      }
      const parsed = JSON.parse(stored) as { id: string; answers: Record<string, number> };
      if (parsed.id !== id) {
        setError('Results ID mismatch. Please take the quiz again.');
        return;
      }
      const computed = calculateResults(parsed.answers);
      setResult(computed);
    } catch {
      setError('Could not load your results. Please take the quiz again.');
    }
  }, [id]);
  ...
  const overallColor = result.overallTier === 'ai-forward'
    ? '#10b981'
    : result.overallTier === 'ready-to-build'
    ? '#10b981'
        ...
        <div style={{ maxWidth: 896, margin: '0 auto' }}>
          ...
          >
            {result.dimensions.map(dim => {
              ...
              const barWidth = ((dim.score - 1) / 4) * 100; // score 1-5 → 0-100%
    ...
    <Suspense
      fallback={
        <div className="flex-1 flex items-center justify-center">
          <p style={{ color: '#a900f1', fontSize: 14 }}>Loading...</p>
        </div>
      }
    >
      <ResultsContent />
